...
function DisplayResults($connectionLink, $query){
    $raw_results2 = mysqli_query($connectionLink,$query) or die(mysqli_error($connectionLink));
    $row2 = mysqli_fetch_assoc($raw_results2);
    $all_favs = $row2['Favourites'];
    if($all_favs == ""){
      return;
    }
    $raw_results = mysqli_query($connectionLink,"SELECT * FROM adverts WHERE id IN ($all_favs)") or die(mysqli_error($connectionLink));
    if(mysqli_num_rows($raw_results) > 0)      
    {
        while($row = mysqli_fetch_assoc($raw_results))
        {
          ...
          if ($row2['visibility'] == 'inactive' || $row2['visibility'] == 'pending'){
              continue;
            }
...
DisplayResults($link, "SELECT Favourites FROM login WHERE ID = '$session_user_id'");
?>
  ...
  <div id="login" class="w3-modal">
    <div class="w3-modal-content w3-card-4 w3-animate-zoom" style="max-width:700px">
      ...
      <div class="w3-container w3-border-top w3-padding-16 w3-light-grey">
        ...
        <span class="w3-right w3-padding w3-hide-small"><a onclick=location.href="passwordReset.php">Nie pamiętasz hasła?</a></span>
